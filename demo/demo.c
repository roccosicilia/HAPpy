...
/*
    ...
    la command line del processo — ed è quindi il target del CLI spoofing.
*/
typedef struct _PEB {
    BYTE Reserved1[2];
    BYTE BeingDebugged;
...
int main() {
    printf("=================================================\n");
    printf(" Test CLI Spoofing + PPID Spoofing\n");
    ...
    */
    ...
    /*
        ...
        componente è utile solo ai fini della demo e non è necessaria
    ...
    */
    ...
    /*
        ============================================================
        STEP 3 — PPID SPOOFING: PREPARA IL PARENT FITTIZIO
        ============================================================
        Il PPID spoofing sfrutta PROC_THREAD_ATTRIBUTE_PARENT_PROCESS,
        un attributo esteso di CreateProcess introdotto in Vista che
        permette di specificare un parent arbitrario al momento della
        creazione del processo.

        Il kernel usa il parent specificato per:
        - registrare il PPID nella struttura EPROCESS del figlio
        - ereditare token e handle (se non diversamente specificato)

        Tool come Process Hacker, Task Manager e la maggior parte
        degli EDR leggono il PPID direttamente da EPROCESS — quindi
        vedranno il parent fittizio, non il processo che ha chiamato
        CreateProcess.

        Il flusso richiede tre chiamate:
        1. OpenProcess: ottiene un handle al parent fittizio con il
           privilegio minimo PROCESS_CREATE_PROCESS
        2. InitializeProcThreadAttributeList (x2): alloca e inizializza
           la lista attributi estesi. La prima chiamata con NULL serve
           solo a ottenere la dimensione del buffer necessario —
           pattern obbligatorio imposto dall'API.
        3. UpdateProcThreadAttribute: inserisce nella lista l'attributo
           PARENT_PROCESS puntando all'handle aperto sopra.

        Il PID hardcoded deve corrispondere a un processo esistente
        e accessibile con i privilegi correnti.
    */
    ...
    printf("[*] PPID spoofato: %lu\n\n", spoofedParentPid);

    // Prima chiamata con NULL: ottiene la dimensione del buffer necessario
    ...
    // Seconda chiamata: inizializza effettivamente la lista
    InitializeProcThreadAttributeList(attrList, 1, 0, &attrListSize);

    // Inserisce l'attributo PARENT_PROCESS nella lista
    ...
    /*
        ============================================================
        STEP 4 — CREA IL PROCESSO SOSPESO CON CMD FALSA E PPID FITTIZIO
        ============================================================
        Il processo viene creato con due tecniche combinate:

        CLI SPOOFING: la command line passata ("Get-Date") è quella
        falsa — visibile a qualsiasi tool che interroghi il processo
        in questo momento (Task Manager, EDR, ETW, ecc.). Verrà
        sovrascritta nel PEB prima che il processo esegua.

        PPID SPOOFING: il flag EXTENDED_STARTUPINFO_PRESENT istruisce
        il kernel a leggere lpAttributeList da STARTUPINFOEXW. Senza
        questo flag la lista viene completamente ignorata.
        STARTUPINFOEXW estende STARTUPINFOA aggiungendo lpAttributeList:
        i campi hStd* si spostano sotto il campo StartupInfo annidato.

        CREATE_SUSPENDED blocca il thread principale prima di qualsiasi
        esecuzione — finestra temporale necessaria per il CLI spoofing.

        Dopo CreateProcess la lista attributi e l'handle al parent
        non servono più e vengono liberati immediatamente.
    */
    STARTUPINFOEXW si = {0};
    ...
    wchar_t fakeCmdLine[] = L"powershell.exe -NoProfile -Command Get-Date";
    ...
    BOOL result = CreateProcessW(
        ...
        EXTENDED_STARTUPINFO_PRESENT, // attiva la lettura di lpAttributeList
        NULL,
        NULL,
        (LPSTARTUPINFOW)&si,  // cast necessario: CreateProcessW vuole la base
        &pi
    );

    // Cleanup immediato: non servono più dopo CreateProcess
    ...
    /*
        ============================================================
        STEP 5 — OTTIENI L'INDIRIZZO BASE DEL PEB DEL FIGLIO
        ============================================================
        NtQueryInformationProcess con classe 0 (ProcessBasicInformation)
    ...
    */
    ...
    /*
        ============================================================
        STEP 6 — LEGGI IL PUNTATORE A ProcessParameters DAL PEB
        ============================================================
    ...
    */
    ...
    /*
        ============================================================
        STEP 7 — LEGGI LA STRUTTURA UNICODE_STRING DELLA COMMANDLINE
        ============================================================
    ...
    */
    ...
    /*
        ============================================================
        STEP 8 — SOVRASCRIVE IL BUFFER CON LA COMMAND LINE REALE
        ============================================================
    ...
    */
    ...
    /*
        ============================================================
        STEP 9 — AGGIORNA IL CAMPO Length DELLA UNICODE_STRING
        ============================================================
    ...
    */
    ...
    /*
        ============================================================
        STEP 10 — RIPRENDE L'ESECUZIONE DEL PROCESSO
        ============================================================
        ResumeThread sblocca il thread principale del processo figlio.
        Da questo momento il processo esegue normalmente, ma trova
        nel suo PEB già la command line reale ("whoami") e come
        parent il processo fittizio specificato al STEP 3.
    ...
    */
    ...
    /*
        ============================================================
        STEP 11 — LEGGI E STAMPA L'OUTPUT REALE DAL PROCESSO
        ============================================================
